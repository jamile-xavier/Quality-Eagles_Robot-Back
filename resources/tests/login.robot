...
TC01 - Realizar login com sucesso Admin
    ...
    ${response}    Realizar login com token admin  email=${MAIL_ADMIN}    senha=${PASSWORD_ADMIN}   expected_status=200
...
TC02 - Realizar login com sucesso User
    ...
    ${response}    Realizar login com token user   email=${MAIL_USER}    senha=${PASSWORD_USER}    expected_status=200
...
TC03 - Realizar login com senha inválida
    ...
    ${response}     Realizar login sem inclusão de token   email=${MAIL_ADMIN}    senha=1234@Tes    expected_status=400
...
TC04 - Realizar login com email inválido
    ...
    ${response}     Realizar login sem inclusão de token   email=sysadmin@qacoders.com.br    senha=${PASSWORD_ADMIN}    expected_status=400
    Status Should Be    400   ${response}
    Should Be Equal    first=E-mail ou senha informados são inválidos.    second=${response.json()["alert"]}

TC05 - Realizar login com email e senha inválidos
    [Documentation]    Realizar login com e-mail e senha inválidos
    ${response}     Realizar login sem inclusão de token   email=sysadmin@qacoders.com.br    senha=1234@Tes    expected_status=400
    Status Should Be    400    ${response}
    Should Be Equal    first=E-mail ou senha informados são inválidos.    second=${response.json()["alert"]}

TC06 - Realizar login com e-mail em branco
    [Documentation]    Realizar login com o e-mail em branco e a senha válida
    ${response}     Realizar login sem inclusão de token    email=       senha=${PASSWORD_ADMIN}    expected_status=400
    Status Should Be    400    ${response}
...
TC07 - Realizar login com a senha em branco
    ...
    ${response}     Realizar login sem inclusão de token   email=${MAIL_ADMIN}    senha=    expected_status=400
    Status Should Be    400    ${response}
...
TC08 - Realizar login com e-mail e senha em branco
    ...
    ${response}     Realizar login sem inclusão de token   email=       senha=    expected_status=400
    Status Should Be    400    ${response}
    Should Be Equal    first=O campo e-mail é obrigatório.    second=${response.json()["mail"]}
    Should Be Equal    first=O campo senha é obrigatório.    second=${response.json()["password"]}
